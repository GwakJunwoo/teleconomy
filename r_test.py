...
df_calendar_te.to_csv('tt.csv', encoding='utf-8-sig')

g20_idx = df_calendar_te[df_calendar_te["지표명"].str.contains("G20", case=False, na=False)].index
ecb_idx = df_calendar_te[df_calendar_te["지표명"].str.contains("Cipollone", case=False, na=False)].index

print("G20 index:", g20_idx)
print("Cipollone index:", ecb_idx)

if not g20_idx.empty and not ecb_idx.empty:
    if g20_idx[0] < ecb_idx[0]:
        print("✅ G20 Meeting이 ECB Cipollone Speech보다 먼저 나옵니다.")
    else:
        print("❌ G20 Meeting이 ECB Cipollone Speech보다 늦게 나옵니다.")
else:
    print("❗ 두 지표 모두 또는 하나가 DataFrame에 존재하지 않습니다.")
